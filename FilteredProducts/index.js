...
let filteredProducts= productDataArr;
...
function Display(data) {
  ...
  data.forEach(function (el) {
    ...
    divv.append(Img, Brand, Price, Catagory);
...
filter.addEventListener("change", function () {
    if(filter.value===""){
      ...
      let filteredP= filteredProducts.filter(function (el) {
    return filter.value === el.catagory;
  });
 Display(filteredP);
}
});
...
if(search.value===null|| search.value===""){
  ...
  let filteredP =filteredProducts.filter(function(el){
  return el.brand.toLocaleLowerCase().includes(search.value.toLocaleLowerCase())
})
 Display(filteredP);
}
})
